...
// Mise à jour: Nouvelle clé API et service ID pour les emails de rejet
...
// Exporter la fonction pour qu'elle soit accessible depuis d'autres composants
export const sendPaymentRejectionEmail = async (participantData: any, rejectionReason: string = '') => {
  try {
    ...
    console.log("Nom du destinataire:", `${participantData.first_name} ${participantData.last_name}`);
    ...
    const formattedPhone = formatPhoneNumber(participantData.contact_number);
...
export const sendMembershipRejectionEmail = async (membershipData: any, rejectionReason: string = '') => {
  try {
    ...
    // Envoi de l'email avec les nouvelles clés API
    const response = await emailjs.send(
      "service_zes2sew", // Nouveau ID du service
      "template_dq0ac69", // Nouveau ID du template
      templateParams,
      "8tZOeeBIj_j__B_cM" // Nouvelle clé publique API
    );
